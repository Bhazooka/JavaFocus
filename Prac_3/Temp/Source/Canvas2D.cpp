...
Canvas2D::Canvas2D(int intRows, int intCols)
{
    ...
    _pixels = new CanvasCapper*[_rows];
    for(int r = 0; r < _rows; r++)
    {
        _pixels[r] = new CanvasCapper[_cols];
        for(int c = 0; c < _cols; c++)
        {
            CanvasCapper BlackCanvas;
...
CanvasCapper Canvas2D::getPixelColour(int intRow, int intCol)
{
...
void Canvas2D:: setPixelColour(int intRow, int intCol, CanvasCapper FaceColour)
{
...
std::string Canvas2D::toPGM()
{
    stringstream ssPGM;
    ssPGM << "P2" << endl           //Header that says this is a colour image
    ...
    for(int r = 0; r < _rows; r++)
    {
        for (int c = 0; c < _cols; c++)
        {
            CanvasCapper objPixel = _pixels[r][c];
            ssPGM << objPixel.getRed() << ' '
                  << objPixel.getGreen() << ' '
                  << objPixel.getBlue() << ' ';
        }
        ssPGM << endl;
...
void Canvas2D::drawFaceCircum(int intRow, int intCol, double dblRadius, CanvasCapper FaceColour)
{
    for(int r = 0; r < _rows; r++)
    {
        for (int c = 0; c < _cols; c++)
