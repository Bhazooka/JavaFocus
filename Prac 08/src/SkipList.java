...
public class SkipList<K, V> implements IDictionary<K ,V> {
    ...
    public SkipList() {
        ...
        insertAfterAbove(start, null, new SkipEntry<K,V>(null, null, MAX));
        size = 0;
        height = 0;
    ...
    */
	...
	//TODO: COMPLETE - 5 MARKS 
        while (true) {
            while (pos.getNext() != null && pos.getNext().element().getKey() != null && comp.compare(pos.getNext().element().getKey(), key) < 0) {
                pos = pos.getNext();
            }
     ...
     */
    protected SLNode<SkipEntry<K,V>> insertAfterAbove(SLNode<SkipEntry<K,V>> before, SLNode<SkipEntry<K,V>> below, SkipEntry<K,V> element) {
        //TODO: COMPLETE - 5 MARKS
    	SLNode<SkipEntry<K,V>> newNode;
        if (before == null || before.getNext() == null) {
            // If before or before.getNext() is null, it means we're inserting at the end of the list
            newNode = new SLNode<>(null, before, null, below, element);
            if (before != null)
                before.setNext(newNode);
        } else {
            newNode = new SLNode<>(before.getNext(), before, null, below, element);
	...
	//TODO: COMPLETE - 10 MARKS
        SLNode<SkipEntry<K,V>> newPos = searchSkip(key);
        if (newPos == null) {
            throw new NoSuchElementException("Key not found: " + key);
        }
        if (newPos.getNext() != null && newPos.getNext().element().getKey() != null && comp.compare(newPos.getNext().element().getKey(), key) == 0) {
            return newPos.getNext().element();
        }
        ...
        while (rand.nextDouble() < 0.5) {
            ...
            while (newPos.getAbove() == null) {
                newPos = newPos.getPrev();
            }
            newPos = newPos.getAbove();
            currentHeight++;
            insertAfterAbove(newPos, newPos.getNext(), newElem);
            newElem.setSentinel(currentHeight);
        }
    ...
    public Iterator<IEntry<K, V>> entries() {
        ...
        ArrayList<IEntry<K, V>> entryList = new ArrayList<>();
        while (current != null && current.element().getKey() != null) {
            entryList.add(current.element());
