...
stcGame* CreateObj(int intRows, int intCols)
{
    ...
    void (*PrintWorldPtr)(stcGame game) = PrintWorld;
    void (*MovePlayerPtr)(stcGame& game, char chInput) = MovePlayer;
    void (*MoveEnemiesPtr)(stcGame& game) = MoveEnemies;
    t_2DArray (*AllocMemPtr)(int intRows, int intCols) = AllocMem;
    
    //initialising the walls of the game
    t_2DArray arrGame = (*AllocMemPtr)(intRows,intCols);
    for(int r=0; r<intRows; r++)
    {
